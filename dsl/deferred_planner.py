"""Helpers for prompting deferred CPL function generation."""
...
_DEFAULT_CONSTRAINTS = (
    "Generate only the function body block (starting and ending with braces).",
...
def build_deferred_prompt(
    context: DeferredFunctionContext,
    dsl_specification: str,
    allowed_syscalls: Iterable[str],
    extra_constraints: Optional[Iterable[str]] = None,
) -> str:
    """Create a natural-language prompt for synthesizing a deferred function body."""

    lines: List[str] = []
    lines.append("You are filling in a deferred Cortex Planning Language (CPL) function body.")
    ...
    lines.append("DSL specification:")
    lines.append(dsl_specification.strip())
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "Respond ONLY with the function body block (including braces) that satisfies these requirements."
    )
    return "\n".join(lines).strip()


def _format_signature(context: DeferredFunctionContext) -> str:
    params = ", ".join(f"{param.name}: {param.type}" for param in context.parameters)
    return f"function {context.function_name}({params}) : {context.return_type}"
...
def prepare_deferred_prompt(
    ...
    prompt = build_deferred_prompt(context, dsl_specification, allowed_syscalls, extra_constraints)
